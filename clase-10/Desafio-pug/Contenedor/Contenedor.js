  ...
  async getAll() {
    try {
      ...
      const allItems = JSON.parse(
        await fs.promises.readFile(`baseDeDatos/${this.fileName}.json`, 'utf-8')
      )

      return allItems
    } catch (error) {
      await fs.promises.writeFile(`baseDeDatos/${this.fileName}.json`, JSON.stringify([]), 'utf-8')
  ...
  async add(object) {
    try {
      ...
      await fs.promises.writeFile(
        `baseDeDatos/${this.fileName}.json`,
